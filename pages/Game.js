...
import useJSON_single from '../pages/useJSON_single';
...
export default function Game({database , user ,data ,playerkey , room , players}) {
    ...
    const [scard]=useJSON_single(database.ref("RoomNames/"+room+"/data/showcard"));
    ...
    function checkwin(){
        ...
        if(threes===1 && fours===1)
            if(!go){
                ...
                exposetoall();
                if(data.wins)
                    database.ref("RoomNames/"+room+"/data/wins/length").set(data.wins.length+1);
                else
                    database.ref("RoomNames/"+room+"/data/wins/length").set(1);
                database.ref("RoomNames/"+room+"/data/wins/"+playerkey).set(new Date().getTime());
            }
    }
    ...
    function UpdateTurn(){
        ...
        if(iszero===0){
            ...
            database.ref("RoomNames/"+room+"/data/hidden/").set(JSON.stringify(hidden));
        }
        database.ref("RoomNames/"+room+"/data/shown/").set(JSON.stringify(shown_cards));
        database.ref("RoomNames/"+room+"/data/myturn/").set(updateturn);
        database.ref("RoomNames/"+room+"/data/showcard/").set(curcard);
    ...
    function initupdateturn(){
        if(data.myturn===Number(playerkey.split("_")[1])){
            ...
            if(data.myturn===newturn){
                database.ref("RoomNames/"+room+"/data/wins/"+playerkey).set(new Date().getTime());
                database.ref("RoomNames/"+room+"/data/status/").set("gameover");
            }
    ...
    */
    function exposetoall(){
        ...
        pos=getpos(pos);
        database.ref("RoomNames/"+room+"/data/showresult/").set({
            name : pos+" : "+user.displayName,
            card : mycards
        });
        setTimeout(()=>{
            database.ref("RoomNames/"+room+"/data/showresult/").remove();
        },5000);
    }
